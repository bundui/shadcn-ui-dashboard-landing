...
/** Minimal slot helper: merges props into its single child element. */
const Slot = React.forwardRef<HTMLElement, SlotProps>((props, forwardedRef) => {
  ...
  merged.ref = composeRefs(forwardedRef, childRef ?? null);
  ...
  const onDragOver = React.useCallback(
    (event: DragOverEvent) => {
      ...
      if (activeColumn === overColumn) {
        ...
        // Insert relative to the hovered item; when hovering the column itself
        // (empty column or below the last card), append to the end.
        ...
        const newIndex =
          overIndex !== -1 ? overIndex + (isBelowOverItem ? 1 : 0) : overItems.length;
    ...
    <KanbanColumnContext.Provider value={columnContext}>
      <SortableContext
        ...
        <ColumnPrimitive
          ...
          style={composedStyle}
          className={cn(
            "bg-muted/50 dark:bg-muted/30 flex size-full flex-col gap-2 rounded-xl border p-2.5 aria-disabled:pointer-events-none aria-disabled:opacity-50",
            {
